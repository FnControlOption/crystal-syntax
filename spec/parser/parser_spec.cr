...
module Syntax
  describe "Parser" do
    ...
    it_parses "def foo(var : self?); end", Def.new("foo", [Arg.new("var", restriction: Syntax::Union.new([Self.new, Path.global("Nil")] of ASTNode))])
    ...
    it_parses "def foo(var : Int | Double); end", Def.new("foo", [Arg.new("var", restriction: Syntax::Union.new(["Int".path, "Double".path] of ASTNode))])
    it_parses "def foo(var : Int?); end", Def.new("foo", [Arg.new("var", restriction: Syntax::Union.new(["Int".path, "Nil".path(true)] of ASTNode))])
    ...
    it_parses "Foo(T | U)", Generic.new("Foo".path, [Syntax::Union.new(["T".path, "U".path] of ASTNode)] of ASTNode)
    it_parses "Foo(Bar(T | U))", Generic.new("Foo".path, [Generic.new("Bar".path, [Syntax::Union.new(["T".path, "U".path] of ASTNode)] of ASTNode)] of ASTNode)
    it_parses "Foo(T?)", Generic.new("Foo".path, [Syntax::Union.new(["T".path, Path.global("Nil")] of ASTNode)] of ASTNode)
    ...
    it_parses "lib LibC\nfun getchar : (->)?\nend", LibDef.new("LibC", [FunDef.new("getchar", return_type: Syntax::Union.new([ProcNotation.new, "Nil".path(true)] of ASTNode))] of ASTNode)
    ...
    it_parses "foo.is_a?(Foo | Bar)", IsA.new("foo".call, Syntax::Union.new(["Foo".path, "Bar".path] of ASTNode))
    it_parses "foo.is_a? Const", IsA.new("foo".call, "Const".path)
    ...
    it_parses "a : Foo | Int32", TypeDeclaration.new("a".var, Syntax::Union.new(["Foo".path, "Int32".path] of ASTNode))
    it_parses "@a : Foo", TypeDeclaration.new("@a".instance_var, "Foo".path)
    it_parses "@a : Foo | Int32", TypeDeclaration.new("@a".instance_var, Syntax::Union.new(["Foo".path, "Int32".path] of ASTNode))
    ...
    it_parses "Foo?", Syntax::Generic.new(Path.global("Union"), ["Foo".path, Path.global("Nil")] of ASTNode)
    it_parses "Foo::Bar?", Syntax::Generic.new(Path.global("Union"), [Path.new(%w(Foo Bar)), Path.global("Nil")] of ASTNode)
    it_parses "Foo(T)?", Syntax::Generic.new(Path.global("Union"), [Generic.new("Foo".path, ["T".path] of ASTNode), Path.global("Nil")] of ASTNode)
    it_parses "Foo??", Syntax::Generic.new(Path.global("Union"), [
      Syntax::Generic.new(Path.global("Union"), ["Foo".path, Path.global("Nil")] of ASTNode),
      Path.global("Nil"),
    ] of ASTNode)
