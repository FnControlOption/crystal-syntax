...
class String
  ...
  def static_array_of(size : ASTNode)
    Generic.new(Syntax::Path.global("StaticArray"), [path, size] of ASTNode)
  end
...
class Syntax::ASTNode
  def pointer_of
    Generic.new(Syntax::Path.global("Pointer"), [self] of ASTNode)
  end
